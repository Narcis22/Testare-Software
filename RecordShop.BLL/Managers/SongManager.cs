...
namespace RecordShop.BLL.Managers
{
    public class SongManager : ISongManager
    {
        ...
        public async Task<List<SongModel>> GetSongsByFilters(int idArtist, int minPrice, int maxPrice)
        {
            if (minPrice <= 0)
            {
                throw new Exception("No negative prices!");
            }
            ...
            if (idArtist <= 0)
            {
                throw new Exception("Must be chosen an artist!");
            }

            var songs = await _songRepository.GetSongsWithArtist()
                .Where(x => x.Artist.Id == idArtist && x.Price >= minPrice && x.Price <= maxPrice)
                .Select(x => new SongModel
                {
                    Id = x.Id,
                    Name = x.Name,
